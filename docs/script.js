...
function onResults(results) {
    ...
    if (results.multiFaceLandmarks) {
        for (const landmarks of results.multiFaceLandmarks) {
            ...
            drawingUtils.drawConnectors(canvasCtx, landmarks, mpFaceMesh.FACEMESH_LEFT_IRIS, { color: "#FFFF00", lineWidth: 1 , fillColor: "#FFFF00"});
        }
    }
    var dx = irisLeftMaxX - irisLeftMinX;
    var dX = 11.7;
    // nom focal length
    var normalizedFocaleX = document.getElementById('focalLength').value/100000;
    ...
    if (dZ === "Infinity") {
        document.getElementById("result").innerHTML = "<i style='color:red'>Please face camera...</i>";
    } else {
        dZ = kf.filter(dZ);
        document.getElementById("result").innerHTML = (dZ/100).toFixed(3) + " m";
    }
    canvasCtx.restore();
}
